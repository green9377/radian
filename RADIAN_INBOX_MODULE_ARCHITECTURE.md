...
| DEC-INB-008 | **(revises DEC-INB-004)** A staff reply does NOT permanently silence the AI. It gives staff PRIORITY: after any new customer message in a staff-engaged thread, the AI waits a grace window (admin-set, default 3 min); if no staff reply lands, the AI answers. Nobody is ever left unanswered. The per-thread AI toggle survives as an explicit owner override (hard off). |
