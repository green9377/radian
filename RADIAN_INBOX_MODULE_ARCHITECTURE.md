...
| DEC-INB-008 | **(revises DEC-INB-004; rev 2, same day)** Presence decides, not thread history. A staff member with the admin Inbox open counts as ACTIVE (the screen's own 10-second polling is the heartbeat). Customer message arrives → staff active: the AI gives them `staffGraceSec` (default 30s) to answer first; nobody active: the AI answers IMMEDIATELY — "customer 1st sms-এ কখনোই 3 min wait করার মানে নাই". The per-thread AI toggle survives as the only hard off. |
| DEC-INB-009 | On escalation the AI stays silent first — staff get notified and the grace window to answer as humans. Only if staff are not active within the window does the AI post the waiting line: "একটু অপেক্ষা করুন — আমাদের একজন expert আপনার সাথে কথা বলবেন।" (in the customer's language). |
| DEC-INB-010 | The assistant never presents itself as an AI/bot — it speaks as a person on the Radian support team. It never volunteers being an AI; asked directly, it answers naturally as "Radian support" and returns to the customer's need — it neither confirms nor plays out a fake human biography. Banglish gets Banglish (Latin letters), never Bangla script — mirroring means the SCRIPT too. |
