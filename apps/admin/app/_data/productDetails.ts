...
/*
  ...
  দাম = size.pricePaisa + bundle.addPaisa + add-ons
...
export interface ProductDetail {
  ...
  crossSlugs: string[];
  ozReason: string;
  reviews: { rating: string; count: number; live: string };
}
...
/* ─────────────────── CATEGORY LABELS ─────────────────── */
const CAT_META: Record<ProductCategory, { label: string; slug: string }> = {
...
function bundleSet(
  ...
  return [
    { id: "none", label: base, addPaisa: 0, bg: GREY },
    ...items.map((i) => ({
      id: i.id,
      label: i.label,
      addPaisa: i.add,
...
const TEMPLATES: Record<ProductCategory, DetailTemplate> = {
  /* ---------------- FLOWERS ---------------- */
  flowers: {
...
export function getProductDetail(slug: string): ProductDetail | null {
  ...
  const base: ProductDetail = {
    ...
    deliveryChip: product.zone === "dhaka" ? "30–120 Min Delivery" : "Delivered Nationwide",
  ...
  return { ...base, ...(DETAIL_OVERRIDES[slug] ?? {}) };
}
