...
export default function ItemListView() {
  ...
  */
  ...
  async function remove(i: ApiItem) {
    ...
    if (u) {
      setErr(
        `“${i.name}” is an ingredient in ${u} recipe${u === 1 ? "" : "s"}. Take it out first.`,
      );
      return;
    }
      ...
      <div className="flex items-end justify-between gap-4 mb-4 flex-wrap">
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => setGenOpen(true)} disabled={busy}
            ...
            title="One item per product that has none">
          ...
          <div className="bg-white rounded-[18px] shadow-lift w-[460px] max-w-full overflow-hidden"
            ...
            <div className="px-5 py-4">
              <p className="text-[13px] text-body m-0">
                One item is created for every product that does not have one yet.
              </p>
            </div>
        ...
        <div className="fixed inset-0 z-[100] grid place-items-center p-4"
          ...
          <div className="bg-white rounded-[18px] shadow-lift w-[520px] max-w-full overflow-hidden"
            ...
            <div className="px-5 py-4">
              {confirming.linked > 0 ? (
                <>
                  <div className="rounded-[12px] px-4 py-3 text-[13px] mb-3" style={{ background: "var(--s-warn)", color: "var(--t-warn)" }}>
                  ...
                  <p className="text-[13px] text-body m-0">
                    {confirming.linked === 1
                      ? "The product keeps its own name, price and photos."
                      : "The products keep their own names, prices and photos."}
                  </p>
                </>
              ) : (
                <p className="text-[13px] text-body m-0">
                  It moves to the trash — you can restore it.
                </p>
              )}
            </div>
          ...
          {items.filter((i) => i.familyKey === photosFam.fkey).map((v) => (
            <div key={v.id} className="flex items-center gap-3 py-2.5 border-b border-lavender-deep/60 last:border-0">
              <label className="relative w-[46px] h-[46px] rounded-[11px] overflow-hidden cursor-pointer grid place-items-center shrink-0"
                style={{ background: v.imageUrl ? "#fff" : "var(--s-accent)", boxShadow: "inset 0 0 0 1px var(--s-accent)" }}
              ...
              {v.imageUrl ? (
                <button onClick={() => setMemberPhoto(v, null, true)} disabled={photoBusyId !== null}
                  className="text-[12px] font-medium underline text-body-soft hover:text-[var(--t-bad)] shrink-0">
                  remove
                </button>
              ) : (
              ...
              return (
                <button key={t} onClick={() => setTab(t)}
                  ...
                  style={on
                    ...
                    : { background: m?.bg ?? "var(--s-accent)", borderColor: "transparent", color: tone }}>
                ...
                return (
                  <button key={o.k} onClick={() => setKind(o.k)}
                    ...
                    <span className="text-[11.5px] font-bold px-1.5 py-[1px] rounded-full"
                      style={on ? { background: "rgba(255,255,255,.22)", color: "#fff" } : { background: "var(--s-accent)", color: ACCENT }}>
                      {kindCounts[o.k]}
                    </span>
                  </button>
                );
          ...
          {FLAGS.map((f) => {
            ...
            return (
              <label key={f.k} className="inline-flex items-center gap-2 cursor-pointer select-none">
                ...
                <span className="text-[13px] font-semibold" style={{ color: on ? ACCENT : "var(--t-accent)" }}>{f.label}</span>
              </label>
            );
          })}
          {anyFilter && (
            <button onClick={clearAll} className="text-[12.5px] font-semibold text-[var(--t-bad)] underline ml-1">
              Clear all
            </button>
          )}
            ...
            if (row.kind === "family") {
              ...
              return (
                <div key={row.fkey} style={{ borderLeft: `4px solid ${fm.colour}` }}>
                  <div onClick={() => toggleFam(row.fkey)}
                    ...
                    <span className="flex items-center justify-end gap-1 text-body-soft">
                      {/*  the whole family in one dialog — price, profit, category,
                           ...
                           (owner, 20 Aug).  */}
                      <button onClick={(e) => { e.stopPropagation(); router.push(`/items/family/${encodeURIComponent(row.fkey)}`); }}
                  ...
                  {open && (
                    <div className="divide-y divide-lavender-deep" style={{ background: "var(--s-accent)" }}>
          ...
          {!loading && shown.length === 0 && (
            <div className="text-center py-14 px-4">
              ...
              <p className="text-[13px] text-body-soft m-0 mt-1">
                {anyFilter ? "Loosen a filter and try again." : "Start with the things you buy."}
              </p>
  ...
  function renderItem(i: ApiItem) {
            ...
            return (
              <div key={i.id} style={{ borderLeft: `4px solid ${i.isActive ? m.colour : "var(--l-accent)"}` }}>
                ...
                >
                  ...
                  <div className="min-w-0">
                    ...
                    <div className="text-[12px] text-body-soft mt-0.5 flex items-center gap-2 flex-wrap">
                      ...
                      {i.isSaleable ? (
                        <>
                          <span className="text-[11px] font-semibold px-1.5 py-0.5 rounded-full" style={{ background: "var(--s-ok)", color: "var(--t-ok)" }}>counter</span>
                          {(i.isOnline ?? true) && (
                            <span className="text-[11px] font-semibold px-1.5 py-0.5 rounded-full" style={{ background: "var(--s-accent)", color: "var(--t-accent)" }}>online</span>
                          )}
                        </>
                      ) : (
                        <span className="text-[11px] font-semibold px-1.5 py-0.5 rounded-full" style={{ background: "var(--s-accent)", color: "var(--t-accent)" }}>not sold</span>
                      )}
                      {i.isSaleable && i.effectiveSellPricePaisa == null && (
                        <span className="text-[11px] font-semibold px-1.5 py-0.5 rounded-full bg-[var(--s-bad)] text-[var(--t-bad)]">no price</span>
                      )}
                      {(i.effectiveCostPaisa ?? 0) <= 0 && !i.isSaleable && <span className="text-[11px] font-semibold px-1.5 py-0.5 rounded-full bg-[var(--s-warn)] text-[var(--t-warn)]">no cost</span>}
                    </div>
                  </div>
                    ...
                    {col ? (
                      <span className="inline-flex items-center gap-1.5 min-w-0">
                        <span className="w-[15px] h-[15px] rounded-full border border-lavender-deep shrink-0"
                          style={{ background: col.swatch ?? "repeating-linear-gradient(45deg,var(--a-solid),var(--a-solid) 4px,var(--a-solid) 4px,var(--a-solid) 8px)" }} />
                  ...
                  <span className="flex items-center justify-end gap-0.5" onClick={(e) => e.stopPropagation()}>
                    ...
                    <button onClick={() => remove(i)} className="text-body-soft hover:text-[var(--t-bad)] px-1 py-1" title="Delete"><Icon name="trash" size={15} /></button>
                  </span>
                </div>
              </div>
            );
