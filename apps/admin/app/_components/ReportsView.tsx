...
const CARD = "bg-white border border-[var(--l-accent)] rounded-[14px] px-4 py-3.5";

function Kpi({ label, value, sub }: { label: string; value: string; sub?: string }) {
  return (
    <div className={CARD}>
      <span className="block text-[11px] font-medium text-[var(--t-accent)] uppercase tracking-[0.04em]">{label}</span>
      <span className="block text-[20px] font-medium text-body mt-1 leading-none">{value}</span>
      {sub && <span className="block text-[11.5px] text-[var(--t-accent)] mt-1.5">{sub}</span>}
    </div>
  );
}
  ...
  return (
    <div className={CARD}>
      <div className="flex items-center justify-between mb-3">
        ...
        {hint && <span className="text-[11.5px] text-[var(--t-accent)]">{hint}</span>}
      </div>
      {children}
    </div>
  );
...
function Bars({ rows, by = "n" }: { rows: ApiOrderReportRow[]; by?: "n" | "revenuePaisa" }) {
  ...
  if (rows.length === 0) return <span className="text-[12.5px] text-[var(--t-accent)]">No data in this range.</span>;
  return (
    <div className="grid gap-2">
      {rows.map((r) => (
        <div key={r.label} className="grid grid-cols-[140px_1fr_120px] items-center gap-2.5 text-[12.5px]">
          ...
          <span className="text-right text-[var(--t-accent)] whitespace-nowrap">
...
function DayChart({ days }: { days: ApiOrderReport["day"] }) {
  if (days.length === 0) return <span className="text-[12.5px] text-[var(--t-accent)]">No data in this range.</span>;
  ...
  return (
    <>
      <div className="flex items-end gap-[5px] h-[130px] border-b border-[var(--l-accent)]">
        {shown.map((d) => (
          <div
            key={d.date}
          ...
          >
            <span className="absolute top-full left-0 right-0 text-center text-[10px] text-[var(--t-accent)] mt-1 whitespace-nowrap">{label(d.date)}</span>
          </div>
        ))}
      </div>
      <span className="block text-[12px] text-[var(--t-accent)] mt-6">
