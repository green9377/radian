...
const KIND_META: Record<string, { icon: string; color: string; bg: string }> = {
  OPENING:      { icon: "book",  color: "var(--t-accent)", bg: "var(--t-accent)" },
  PURCHASE:     { icon: "box",   color: "var(--t-accent)", bg: "var(--t-accent)" },
  PAYMENT:      { icon: "cash",  color: "var(--t-ok)", bg: "var(--t-ok)" },
  BILL_PAYMENT: { icon: "cash",  color: "var(--t-ok)", bg: "var(--t-ok)" },
  RETURN:       { icon: "truck", color: "var(--t-warn)", bg: "var(--t-warn)" },
  ADJUSTMENT:   { icon: "edit",  color: "var(--t-info)", bg: "var(--t-info)" },
};
  ...
  return (
    <div className={WRAP}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-5">
        {[
          { l: "Due (we owe)", v: formatTaka(s.duePaisa), c: s.duePaisa > 0 ? "var(--t-bad)" : "var(--t-ok)", bg: s.duePaisa > 0 ? "var(--s-bad)" : "var(--s-ok)", icon: "wallet" },
          { l: "Credit we hold", v: formatTaka(s.creditPaisa), c: "var(--t-ok)", bg: "var(--s-ok)", icon: "gem" },
          { l: "Bought (all time)", v: formatTaka(s.totalBoughtPaisa), c: "var(--t-accent)", bg: "var(--s-accent)", icon: "cart" },
          { l: "Purchases", v: String(s.purchaseCount), sub: s.lastPurchaseAt ? `last ${fmtDate(s.lastPurchaseAt)}` : "", c: "var(--t-info)", bg: "var(--s-info)", icon: "box" },
        ].map((k) => (
        ...
        <div>
          ...
          {tab === "ledger" && ledger && (
            <div className="bg-white border border-lavender-deep rounded-[16px] shadow-soft overflow-hidden">
              <div className="flex items-center justify-between pl-4 pr-4 py-2" style={{ background: "var(--s-accent)" }}>
                      ...
                      <div key={i}>
                        {showDay && (
                          <div className={"flex items-center gap-2 " + (i === 0 ? "pt-1" : "pt-3")}>
                            <span className="text-[10.5px] font-bold uppercase tracking-[0.06em] px-2 py-0.5 rounded-full"
                              style={{ background: "var(--s-accent)", color: "var(--t-accent)" }}>{day}</span>
                            <span className="flex-1 h-px" style={{ background: "var(--s-accent)" }} />
                          </div>
                        )}
                        <div className="flex items-center gap-3 py-2.5 border-b border-lavender-deep/50 last:border-0">
                          ...
                          <span className="text-right shrink-0">
                            <b className="block text-[14px]" style={{ color: e.amountPaisa >= 0 ? "var(--t-bad)" : "var(--t-ok)", fontVariantNumeric: "tabular-nums" }}>
              ...
              {ledger.months.length > 0 && (
                <div className="px-4 py-3 border-t border-lavender-deep bg-[var(--s-accent)]">
                  ...
                  {ledger.months.map((m) => (
                    <div key={m.month} className="flex items-center justify-between text-[12px] py-1">
                      ...
                      <span className="flex gap-2">
                        <span className="px-2 py-0.5 rounded-full" style={{ background: "var(--s-accent)", color: "var(--t-accent)" }}>
                          bought <b style={{ fontVariantNumeric: "tabular-nums" }}>{formatTaka(m.bought)}</b>
                        </span>
                        <span className="px-2 py-0.5 rounded-full" style={{ background: "var(--s-ok)", color: "var(--t-ok)" }}>
                          paid <b style={{ fontVariantNumeric: "tabular-nums" }}>{formatTaka(m.paid)}</b>
                        </span>
                        {m.returned > 0 && (
                          <span className="px-2 py-0.5 rounded-full" style={{ background: "var(--s-warn)", color: "var(--t-warn)" }}>
                    ...
                    return (
                      <Link key={it.id} href={`/items/${it.id}`}
                        ...
                        <span className="text-[13px] font-semibold text-right"
                          style={{ color: margin == null ? "var(--t-accent)" : margin >= 0 ? "var(--t-ok)" : "var(--t-bad)" }}>
                ...
                return (
                  <div key={c.id} className="flex items-center justify-between gap-3 py-2 border-b border-lavender-deep/60 last:border-0">
                    ...
                    <span className="flex items-center gap-2 shrink-0">
                      <b className="text-[13px]" style={{ color: consumed ? "var(--t-accent)" : "var(--t-ok)" }}>{formatTaka(c.amountPaisa)}</b>
                      {!consumed && (
                        <button
                          onClick={async () => {
  ...
  return (
    <Modal title={`Pay ${s.nickname || s.name}`} onClose={onClose}>
      ...
      <p className="text-[12.5px] text-body-soft mt-0 mb-3">
        Current due <b style={{ color: "var(--t-bad)" }}>{formatTaka(s.duePaisa)}</b>
        {s.creditPaisa > 0 && <> · credit already held <b style={{ color: "var(--t-ok)" }}>{formatTaka(s.creditPaisa)}</b></>}
        ...
        <>
          ...
          <div className="rounded-[12px] border border-lavender-deep px-4 py-3 mb-3">
            ...
            <div className="flex items-center justify-between pt-2 mt-1 border-t border-lavender-deep/60 text-[12.5px]">
              ...
              {excess > 0 && <b style={{ color: "var(--t-ok)" }}>{formatTaka(excess)} → credit (no cash back)</b>}
              {allocated > amount && <b style={{ color: "var(--t-bad)" }}>over by {formatTaka(allocated - amount)}</b>}
            </div>
          </div>
        ...
        <div className="flex rounded-[10px] border border-lavender-deep overflow-hidden shrink-0 mt-[26px]">
          {([-1, 1] as const).map((v) => (
            <button key={v} type="button" onClick={() => setSign(v)}
              ...
              style={sign === v ? { background: ACCENT, color: "#fff" } : { background: "#fff", color: "var(--t-accent)" }}>
              {v === -1 ? "−" : "+"}
            </button>
          ))}
        </div>
...
function OrderMessageModal({ s, onClose }: { s: ApiSupplierDetail; onClose: () => void }) {
  ...
  return (
    <Modal title={`Message ${s.nickname || s.name}`} onClose={onClose}>
      ...
      <div className="rounded-[12px] px-4 py-3 mb-3 text-[12.5px] text-body" style={{ background: "var(--s-accent)" }}>{text}</div>
      <a href={href} target="_blank" rel="noreferrer"
        className="block text-center text-white text-[13.5px] font-semibold px-4 py-3 rounded-[12px]"
        style={{ background: s.notifyChannel === "WHATSAPP" ? "var(--s-ok)" : ACCENT }}>
