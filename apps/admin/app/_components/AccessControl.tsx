...
const DEPT: Record<string, { text: string; grad: string; icon: string }> = {
  "Today's work":   { text: "#d38899", grad: "linear-gradient(120deg,#c25a72,#451e27)", icon: "clock" },
  "What you sell":  { text: "#d475e6", grad: "linear-gradient(120deg,#a021b8,#e07be0)", icon: "star" },
  "Stock & buying": { text: "#75f0c7", grad: "linear-gradient(120deg,#12a172,#5ec9a8)", icon: "box" },
  "Money":          { text: "#edc278", grad: "linear-gradient(120deg,#b07818,#e9c46a)", icon: "cash" },
  "Growth":         { text: "#82a7d9", grad: "linear-gradient(120deg,#3b76c4,#7fb4f0)", icon: "chart" },
  "Setup":          { text: "#a8a1ba", grad: "linear-gradient(120deg,#7a6f96,#322d3c)", icon: "gear" },
};
    ...
    <div className={WRAP}>
            ...
            {position && !position.isOwner && (
              <span className="text-[11px] font-bold px-2.5 py-1 rounded-full"
                style={{
                  ...
                  color: saveState === "error" ? "#fff" : "#b97fdc",
                }}>
      ...
      <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mb-3 px-1 text-[11.5px] font-semibold">
        <span className="flex items-center gap-1.5">
          ...
          <span className="text-[#dfd2e4]">Allow — you said open</span>
        </span>
        <span className="flex items-center gap-1.5">
          <span className="w-[10px] h-[10px] rounded-[3px]" style={{ background: "#5b5370" }} />
          <span className="text-[#dfd2e4]">Auto — no rule of its own, follows its module</span>
        </span>
        <span className="flex items-center gap-1.5">
          <span className="w-[10px] h-[10px] rounded-[3px]" style={{ background: "#d94838" }} />
          <span className="text-[#dfd2e4]">Block — you said shut</span>
        </span>
      </div>
      ...
      <div className="grid gap-4 lg:grid-cols-[236px_minmax(0,1fr)] items-start">
        ...
        <div className="lg:sticky lg:top-4 space-y-2">
          <div className="rounded-[16px] bg-white border border-[#3c3249] overflow-hidden"
            ...
            {adding && (
              <div className="p-2.5 border-b border-[#3c2f4b] flex gap-1.5">
                ...
                return (
                  <button key={p.id} onClick={() => setSelected(p.id)}
                    ...
                    <span className="w-[26px] h-[26px] rounded-[8px] grid place-items-center text-[12px] font-bold shrink-0"
                      style={{
                        background: on ? "rgba(255,255,255,0.25)" : p.isOwner ? "linear-gradient(135deg,#b76e79,#432926)" : "#2b1d37",
                        color: on ? "#fff" : p.isOwner ? "#fff" : "#b97fdc",
                      }}>
                      {p.isOwner ? <Icon name="lock" size={12} /> : p.name.slice(0, 1).toUpperCase()}
                    </span>
                    <span className="min-w-0 flex-1">
                      <span className={"block text-[13px] font-bold truncate " + (on ? "text-white" : "text-[#dfd2e4]")}>{p.name}</span>
                      <span className={"block text-[10.5px] font-medium " + (on ? "text-white/75" : "text-[#aaa4b7]")}>
            ...
            {position && !position.isOwner && renaming?.id === position.id && (
              <div className="px-3 py-2 border-t border-[#3c2f4b] flex items-center gap-2">
                <input
                  autoFocus
            ...
            {position && !position.isOwner && renaming?.id !== position.id && (
              <div className="px-3 py-2 border-t border-[#3c2f4b] flex gap-3">
                ...
                {!position.isLocked && (
                  <button className="text-[11px] font-bold hover:underline" style={{ color: "#e1837a" }} onClick={() => void drop(position)}>Delete</button>
                )}
              </div>
            )}
          </div>

          <p className="text-[11px] text-[#aaa4b7] leading-relaxed px-1 m-0">
        ...
        {!position ? (
          ...
          <div className="rounded-[16px] bg-white border border-[#3c3249] overflow-hidden"
            style={{ boxShadow: "0 2px 10px rgba(70,0,102,0.06)" }}>
            {position.isOwner && (
              <div className="px-4 py-2.5 flex items-center gap-2.5 border-b border-[#3c2f4b]"
                style={{ background: "#37221a" }}>
                <Icon name="lock" size={14} strokeWidth={2.4} />
                <span className="text-[12px] font-semibold" style={{ color: "#c299a1" }}>
            ...
            <div className="px-4 py-3 border-b border-[#3c2f4b] flex items-center gap-3 flex-wrap"
              style={{ background: "#271a34" }}>
              <span className="text-[12.5px] font-bold text-purple">See cost prices</span>
              <span className="text-[11.5px] text-[#aaa4b7] flex-1 min-w-[220px]">
                What the shop paid — on items, at the till, in reports. The selling price is always visible.
              </span>
              {position.isOwner ? (
                <span className="text-[11px] font-bold px-2.5 py-1 rounded-full"
                  style={{ background: "#20332e", color: "#74f1d7" }}>Always on</span>
              ) : (
                <button
                ...
                <div key={domain}>
                  ...
                  {mods.map((mod) => {
                    ...
                    return (
                      <div key={mod.key}>
                        <div
                          className="flex items-center gap-2.5 pr-3 py-[9px] border-b border-[#3c3149] hover:bg-[#271c32] transition-colors"
                          ...
                          <button
                            ...
                            {mod.children.length > 0 ? (
                              <span className="w-[22px] h-[22px] rounded-[7px] grid place-items-center shrink-0 transition-colors"
                                style={{ background: isOpen ? c.text : "#282033" }}>
                              ...
                              <span className="w-[22px] grid place-items-center shrink-0">
                                <span className="w-[7px] h-[7px] rounded-full"
                                  style={{ background: eff.allowed ? c.text : "#2d2935" }} />
                              </span>
                            )}
                            <span className="text-[14px] font-bold truncate text-[#dfd2e4]">
                              {mod.label}
                            </span>
                            {mod.children.length > 0 && (
                              <span className="text-[10px] font-bold px-1.5 py-[1px] rounded-full shrink-0"
                                style={{ background: "#282033", color: "#aaa3b8" }}>
                                {mod.children.length}
                              </span>
                            )}
                            <span className="text-[10px] font-bold px-1.5 py-[1px] rounded-full shrink-0"
                              style={{
                                background: eff.allowed ? "#1b372b" : "#272130",
                                color: eff.allowed ? "#75f0c7" : "#9a91ac",
                              }}>
...
function ScreenRow({
  ...
  return (
    <>
      <div
        className="flex items-center gap-2 pr-3 py-[6px] border-b border-[#3c3347] last:border-0"
        style={{ paddingLeft: 34 + depth * 18 }}
      >
        <span className="w-[7px] h-[7px] rounded-full shrink-0"
          style={{ background: eff.allowed ? accent : "#2d2935" }} />
        <span className="text-[13px] font-semibold flex-1 truncate text-[#dfd2e4]">
          {node.label}
          {explicit && (
            <span className="text-[9.5px] font-bold ml-1.5 px-1.5 py-[1px] rounded-full align-middle"
              style={{ background: "#282033", color: "#a8a1ba" }}>own rule</span>
          )}
        </span>
        <TriState
