    ...
    if (editing) {
      ...
      if (changed && used > 0) {
        const ok = confirm(
          ...
          `Changing the conversion re-reads every stock figure counted in this unit — ` +
          `what was 40 papri may now mean something else.\n\nContinue?`,
        );
        if (!ok) return;
      }
    }
      ...
      {isDemo && (
        <div className="flex items-center gap-3 bg-[#fff4e6] border border-[#fce4c4] text-[#b45309] rounded-[12px] px-4 py-2.5 mb-4 text-[12.5px] flex-wrap">
          ...
          <span className="flex-1 min-w-[220px]">
            The API (:4000) is not reachable — showing sample units. Start the API and
  ...
  return (
    <div className={ROW + " px-4 py-3 " + (unit.isActive ? "hover:bg-lavender/25" : "bg-[#fbf5ef] hover:bg-[#f7efe7]")}>
      ...
      <div className="flex items-center gap-1 justify-end">
        <button onClick={onDuplicate} className="w-[30px] h-[30px] rounded-[8px] grid place-items-center text-purple hover:bg-lavender" title="Duplicate — same conversion, new name"><Icon name="copy" size={14} /></button>
          ...
          <div>
            ...
            {mode === "duplicate" && (
              <div className="text-[13px] text-body-soft mt-0.5">
                Conversion copied — give it the new flower’s name and check the number.
              </div>
            )}
          </div>
      ...
      <div className="bg-white w-full max-w-[560px] h-full flex flex-col shadow-soft" onClick={(e) => e.stopPropagation()}>
        <div className="px-5 py-4 border-b border-lavender-deep flex items-start justify-between gap-3" style={{ background: ACCENT_BG }}>
          <div>
            ...
            <div className="text-[13px] text-body-soft mt-0.5">
              Pick what to move, choose the new unit, and each one is updated through its own module.
            </div>
          </div>
