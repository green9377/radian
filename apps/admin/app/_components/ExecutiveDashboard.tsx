...
function Jump({ id, label, icon }: { id: string; label: string; icon: string }) {
  return (
    <a
      ...
      style={{ borderColor: "var(--l-accent)", color: "var(--t-accent)", background: "var(--s-accent)" }}
    >
...
export function ExecutiveDashboard() {
  ...
  return (
    <div className={WRAP}>
      <Header
        ...
        actions={
          <div className="flex items-center gap-2">
            ...
            {data?.seesMoney && (
              <>
                <Link href="/intelligence/analytics" className="rounded-[10px] border border-[var(--l-accent)] bg-white px-3 py-1.5 text-[12.5px] text-purple hover:bg-[var(--s-accent)]">
                  Analytics
                </Link>
                <Link href="/intelligence/reports" className="rounded-[10px] border border-[var(--l-accent)] bg-white px-3 py-1.5 text-[12.5px] text-purple hover:bg-[var(--s-accent)]">
                  Reports
                </Link>
              </>
            )}
            <button
              onClick={() => void load()}
              className="rounded-[10px] border border-[var(--l-accent)] bg-white px-3 py-1.5 text-[12.5px] text-purple hover:bg-[var(--s-accent)]"
            >
      ...
      {clear ? (
        <div className="rounded-[16px] border border-[var(--l-ok)] bg-[var(--s-ok)] px-4 py-5 text-[13.5px] text-[var(--t-ok)]">
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-2.5 mt-3">
        {data.business.supporting.map((s) => (
          <div key={s.key} className="rounded-[14px] border border-[var(--l-accent)] bg-white px-3.5 py-3">
      ...
      {data.seesMoney && (() => {
        ...
        if (!data.meta.historyReady || active.length === 0) {
          return (
            <div className="mt-3 rounded-[14px] border border-[var(--l-accent)] bg-[var(--s-accent)] px-4 py-3 text-[12.5px] text-purple leading-relaxed">
                ...
                {days.map((d) => (
                  <div
                    ...
                    style={{
                      ...
                      background: d.revenuePaisa > 0 ? "var(--s-info)" : "var(--s-info)",
                    }}
                  />
                ))}
              </div>
      ...
      {data.money ? (
        <>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-2.5">
            {data.money.figures.map((f) => (
              <Link
                key={f.key}
                href={f.href}
                className="rounded-[14px] border border-[var(--l-accent)] bg-white px-3.5 py-3 transition-transform hover:-translate-y-0.5"
              >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-3">
            <div className="rounded-[14px] border border-[var(--l-accent)] bg-white px-4 py-3.5">
              ...
              {data.money.breakEven.known ? (
                <>
                  ...
                  <div className="mt-2.5 h-[6px] rounded-full overflow-hidden bg-[var(--s-accent)]">
                    <div className="h-full rounded-full bg-[var(--s-accent)]" style={{ width: `${Math.min(100, data.money.breakEven.progressBp / 100)}%` }} />
                  </div>
                </>
              ) : (
