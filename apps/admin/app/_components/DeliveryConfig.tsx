...
export function CouriersMaster() {
  ...
  const stats = [
    { l: "Riders (in-house)", v: String(riders.length), c: "var(--t-accent)", bg: "var(--s-accent)", icon: "user" },
    { l: "3PL couriers", v: String(couriers.length), c: "var(--t-info)", bg: "var(--s-info)", icon: "truck" },
    { l: "Active", v: String(fleet.filter((f) => f.active).length), c: "var(--t-ok)", bg: "var(--s-ok)", icon: "check" },
    { l: "API connected", v: String(couriers.filter((f) => f.apiConfigured).length), c: "var(--t-warn)", bg: "var(--s-warn)", icon: "bolt" },
    { l: "Deliveries 30d", v: String(fleet.reduce((n, f) => n + f.deliveries30d, 0)), c: "var(--t-accent)", bg: "var(--s-accent)", icon: "box" },
    { l: "Avg on-time", v: (Math.round(fleet.filter((f) => f.onTimePct).reduce((n, f) => n + f.onTimePct, 0) / Math.max(1, fleet.filter((f) => f.onTimePct).length))) + "%", c: "var(--t-ok)", bg: "var(--s-ok)", icon: "clock" },
  ];

  const Rowlet = ({ f }: { f: FleetMember }) => {
    const on = sel === f.id;
    return (
      <div className={"grid grid-cols-[1fr_auto] items-center gap-2 rounded-[11px] px-3 py-2.5 cursor-pointer border " + (on ? "bg-orchid-soft border-orchid" : f.active ? "bg-white border-lavender-deep hover:border-orchid" : "bg-white border-[var(--l-warn)]")} onClick={() => setSel(f.id)}>
        <div className="flex items-center gap-2.5 min-w-0">
          ...
          <div className="min-w-0">
            <div className="text-[13.5px] font-medium text-purple truncate">{f.name}{!f.active && <span className="ml-1.5 text-[10px] font-semibold px-1.5 py-0.5 rounded-full bg-[var(--s-warn)] text-[var(--t-warn)]">Off</span>}</div>
    ...
    <EditorShell
      ...
      footer={!isNew && onDelete ? (
        <div className="flex justify-between items-center pt-2 border-t border-lavender-deep">
          <button onClick={onDelete} className="text-[13px] font-medium text-[var(--t-bad)] hover:underline inline-flex items-center gap-1.5"><Icon name="trash" size={15} /> Remove</button>
...
export function ZonesRates() {
  ...
  const stats = [
    { l: "Zones", v: String(zones.length), c: "var(--t-accent)", bg: "var(--s-accent)", icon: "pin" },
    { l: "Active types", v: String(rates.filter((r) => r.active).length), c: "var(--t-info)", bg: "var(--s-info)", icon: "bolt" },
    { l: "Time slots", v: String(slots.length), c: "var(--t-accent)", bg: "var(--s-accent)", icon: "clock" },
    { l: "Avg charge", v: formatTaka(Math.round(rates.reduce((n, r) => n + r.chargePaisa, 0) / Math.max(1, rates.length))), c: "var(--t-ok)", bg: "var(--s-ok)", icon: "cash" },
    { l: "Slot capacity", v: String(slots.reduce((n, s) => n + s.capacity, 0)), c: "var(--t-warn)", bg: "var(--s-warn)", icon: "box" },
    { l: "Blackout dates", v: String(DEMO_BLACKOUTS.length), c: "var(--t-warn)", bg: "var(--s-warn)", icon: "shield" },
  ];
      ...
      <div className="grid grid-cols-1 xl:grid-cols-[minmax(300px,1fr)_1.6fr] gap-6 items-start">
        <div className="space-y-2 xl:sticky xl:top-4">
          {zones.map((z) => {
            ...
            return (
              <div key={z.id} className={"rounded-[12px] px-3.5 py-3 cursor-pointer border " + (on ? "bg-orchid-soft border-orchid" : z.active ? "bg-white border-lavender-deep hover:border-orchid" : "bg-white border-[var(--l-warn)]")} onClick={() => setSel(z.id)}>
                <div className="flex items-center gap-2">
                  ...
                  {!z.active && <span className="text-[10px] font-semibold px-1.5 py-0.5 rounded-full bg-[var(--s-warn)] text-[var(--t-warn)]">Off</span>}
                </div>
              </div>
            );
          })}
...
function ZoneEditor({ zone, rates, slots, onSave, onDelete, onCancel }: {
  ...
  return (
    <EditorShell
      ...
      footer={!isNew && onDelete ? (
        <div className="flex justify-between items-center pt-2 border-t border-lavender-deep">
          <button onClick={onDelete} className="text-[13px] font-medium text-[var(--t-bad)] hover:underline inline-flex items-center gap-1.5"><Icon name="trash" size={15} /> Remove zone</button>
          <button onClick={submit} disabled={!name.trim()} className="bg-purple hover:bg-purple-deep disabled:opacity-60 text-white text-[13.5px] font-medium px-6 py-2.5 rounded-[11px] shadow-soft inline-flex items-center gap-1.5"><Icon name="check" size={15} /> Save</button>
        </div>
      ) : null}
    >
      ...
      <Section title="Delivery types & charges" icon="bolt">
        <div className="space-y-2">
          {TYPES.map((t) => {
            ...
            return (
              <div key={t} className="rounded-[12px] border px-3 py-2.5" style={{ borderColor: d.on ? tn.border: "var(--l-accent)", background: d.on ? tn.bg : "var(--s-accent)" }}>
        ...
        <div className="space-y-2">
          ...
          {zs.map((s) => (
            <div key={s.id} className="grid grid-cols-[1fr_1fr_auto_auto_auto] gap-2 items-center">
              ...
              <button onClick={() => delSlot(s.id)} className="text-body-soft hover:text-[var(--t-bad)]" title="remove"><Icon name="trash" size={16} /></button>
            </div>
          ))}
