      ...
      >
        {rows.map((c) => (
          <div key={c.id} className={ROW + " py-2.5 hover:bg-lavender/15"}>
            ...
            <span>
              {c.isActive
                ? <span className="text-[11px] font-semibold px-2 py-0.5 rounded-full" style={{ background: "var(--s-ok)", color: "var(--t-ok)" }}>on</span>
                : <span className="text-[11px] font-semibold px-2 py-0.5 rounded-full" style={{ background: "var(--s-accent)", color: "var(--t-accent)" }}>off</span>}
            </span>
            ...
            {c.slug === "website" ? (
              <span className="flex items-center justify-end gap-1.5 text-body-soft">
              ...
              <span className="flex items-center justify-end gap-1">
                ...
                <button onClick={() => void remove(c)}
                  className="text-body-soft hover:text-[var(--t-bad)] px-1.5 py-1" title="Remove"><Icon name="trash" size={15} /></button>
              </span>
            )}
          </div>
        ))}
