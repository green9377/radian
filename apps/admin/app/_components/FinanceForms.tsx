...
export function ExpensesLive() {
  ...
  return (
    <div className={WRAP}>
      ...
      <Card className="px-5 py-4 mb-6" tone="brand" style={{ background: "var(--s-accent)" }}>
        ...
        <div className="grid md:grid-cols-6 gap-3 items-end mt-3">
          ...
          <div className="md:col-span-2 flex items-center gap-3">
            ...
            {willNeedApproval && <span className="text-[12px] text-[var(--t-warn)] font-semibold">Needs approval</span>}
          </div>
        </div>
      </Card>
      ...
      {pending.length > 0 && (
        <>
          <h2 className="font-display text-[19px] text-purple mb-3 flex items-center gap-2"><span className="w-1.5 h-5 rounded-full" style={{ background: "linear-gradient(180deg,var(--o-solid),var(--o-solid))" }} />Waiting for approval</h2>
          <Card className="overflow-hidden mb-7">
            {pending.map((r) => (
              <div key={r.id} className="flex items-center justify-between gap-4 px-4 py-3 border-b border-[var(--l-accent)] last:border-0">
                <div>
      ...
      <h2 className="font-display text-[19px] text-purple mb-3 flex items-center gap-2"><span className="w-1.5 h-5 rounded-full" style={{ background: "linear-gradient(180deg,var(--o-solid),var(--o-solid))" }} />Recorded</h2>
            ...
            {rows.map((r) => (
              <tr key={r.id} className="border-t border-[var(--l-accent)]">
                ...
                <td className="px-4 py-2.5 text-right">
                  {r.journalEntryId ? <span className="text-[var(--t-ok)] font-semibold">In the books</span>
                    : r.approval === "PENDING" ? <span className="text-[var(--t-warn)] font-semibold">Waiting</span>
                    : <span className="text-body-soft">{r.approval.toLowerCase()}</span>}
                  {!r.journalEntryId && r.approval !== "PENDING" && (
                    <button className="ml-2 text-[12px] text-[var(--t-bad)] font-semibold"
...
export function IncomeLive() {
  ...
  return (
    <div className={WRAP}>
      ...
      <h2 className="font-display text-[19px] text-purple mb-3 flex items-center gap-2"><span className="w-1.5 h-5 rounded-full" style={{ background: "linear-gradient(180deg,var(--o-solid),var(--o-solid))" }} />Move money between your accounts</h2>
      ...
      {trs.length > 0 && (
        <Card className="overflow-hidden mb-8">
          <table className="w-full text-[13px]">
            <tbody>
              {trs.map((t) => (
                <tr key={t.id} className="border-b border-[var(--l-accent)] last:border-0">
      ...
      <h2 className="font-display text-[19px] text-purple mb-3 flex items-center gap-2"><span className="w-1.5 h-5 rounded-full" style={{ background: "linear-gradient(180deg,var(--o-solid),var(--o-solid))" }} />Other income</h2>
      <Card className="px-5 py-4 mb-6" tone="brand" style={{ background: "var(--s-accent)" }}>
        <div className="grid md:grid-cols-6 gap-3 items-end">
          <div>
            <label className="text-[12px] font-semibold text-body-soft">Date</label>
      ...
      <Card className="overflow-hidden">
        <table className="w-full text-[13px]">
          <tbody>
            ...
            {rows.map((r) => (
              <tr key={r.id} className="border-b border-[var(--l-accent)] last:border-0">
    ...
    <div className={WRAP}>
        ...
        {rows.map((p) => (
          <Card key={p.id} className="px-5 py-4">
            <div className="flex items-start justify-between gap-3">
              ...
              <select className="border border-[var(--l-accent)] rounded-lg px-2 py-1 text-[12px] bg-white"
            ...
            <div className="grid grid-cols-2 gap-3 mt-4">
              ...
              <div>
                ...
                <div className="text-[17px] font-bold" style={{ color: p.capitalOutstandingPaisa > 0 ? "var(--t-warn)" : "var(--t-ok)" }}>
            ...
            {p.transactions.length > 0 && (
              <div className="mt-4 border-t border-[var(--l-accent)] pt-3">
      ...
      {rows.length > 0 && (
        <Card className="px-5 py-4 mt-5">
          ...
          {share && (
            <div className="mt-4 border-t border-[var(--l-accent)] pt-4">
              <div className="text-[13px] mb-2">
                This month left over: <b style={{ color: share.profitPaisa < 0 ? "var(--t-bad)" : "var(--t-ok)" }}>{taka(share.profitPaisa)}</b>
              ...
              {share.profitPaisa <= 0 ? (
                ...
                <>
                  ...
                  <div className="mt-3 flex gap-2 items-center">
                    ...
                    {!share.enoughCash && <span className="text-[12.5px] text-[var(--t-bad)] font-semibold">Not enough cash in hand to pay this</span>}
                  </div>
                </>
              )}
            </div>
      ...
      {txn && (
        <Card className="px-5 py-4 mt-5 border-[var(--l-accent)]">
