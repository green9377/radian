...
const SYSTEM_THEME: Record<string, GroupTheme> = {
  occasions: { c: "var(--t-accent)", bg: "var(--s-accent)", edge: "var(--l-accent)", icon: "sparkle" },
  recipients: { c: "var(--t-gold)", bg: "var(--s-bad)", edge: "var(--l-gold)", icon: "user" },
};
const CUSTOM_THEMES: GroupTheme[] = [
  { c: "var(--t-accent)", bg: "var(--s-accent)", edge: "var(--l-accent)", icon: "tag" },
  { c: "var(--t-gold)", bg: "var(--s-bad)", edge: "var(--l-bad)", icon: "heart" },
  { c: "var(--t-accent)", bg: "var(--s-accent)", edge: "var(--l-orchid)", icon: "star" },
  { c: "var(--t-accent)", bg: "var(--s-accent)", edge: "var(--l-accent)", icon: "grid" },
];
  ...
  return (
    <div className={WRAP}>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3.5 mb-6">
        {[
          { l: "Groups", v: stats.groups, c: "var(--t-accent)", edge: "var(--l-accent)", bg: "var(--s-accent)", icon: "layers" },
          { l: "Tags", v: stats.tags, c: "var(--t-accent)", edge: "var(--l-orchid)", bg: "var(--s-accent)", icon: "tag" },
          { l: "Unused", v: stats.empty, c: "var(--t-gold)", edge: "var(--l-gold)", bg: "var(--s-bad)", icon: "bolt", tip: "Tags no product carries yet." },
          { l: "Hidden", v: stats.hidden, c: "var(--t-accent)", edge: "var(--l-accent)", bg: "var(--s-accent)", icon: "eye", tip: "Switched off — hidden from the shop, kept here." },
        ].map((k, i) => (
                    ...
                    <div className="flex items-center gap-1.5 flex-wrap">
                      ...
                      {!g.isActive && (
                        <span className="shrink-0" title="Hidden from the storefront">
                          <Icon name="eye" size={12} className={on ? "text-[var(--t-warn)]" : "text-white/50"} />
                        </span>
                      )}
                    </div>
  ...
  return (
    <div className="bg-white rounded-[18px] shadow-soft overflow-hidden" style={{ border: `1px solid ${theme.edge}40` }}>
      ...
      <div className="grid grid-cols-[auto_1fr_auto] items-center gap-3 px-5 py-4 border-b relative"
        ...
        <div className="flex items-center gap-2 shrink-0">
          {g.isSystem ? (
            ...
            <button onClick={onDeleteGroup} className="w-[34px] h-[34px] rounded-[9px] grid place-items-center text-[var(--t-bad)] bg-white/70 hover:bg-white" title="Delete group"><Icon name="trash" size={15} /></button>
          )}
      ...
      <div className="px-5 py-3.5 border-b border-lavender-deep flex items-center gap-3 flex-wrap"
        style={{ background: "linear-gradient(180deg,#ffffff,var(--a-solid))" }}>
      ...
      {isCard && missingImages > 0 && (
        <div className="mx-3.5 mt-3 bg-[var(--s-warn)] border border-[var(--l-warn)] text-[var(--t-warn)] rounded-[10px] px-3 py-2 text-[12px] flex items-center gap-2">
              ...
              return (
                <div key={t.id}
                  ...
                  style={t.isActive
                    ...
                    : { background: "var(--s-accent)", borderColor: "var(--l-accent)", opacity: 0.75 }}>
                  {isCard && (
                    <div className="relative shrink-0">
                      ...
                      {t.img && (
                        <button onClick={() => onClearImage(t)} className="absolute -top-1.5 -right-1.5 w-[18px] h-[18px] rounded-full bg-white border border-lavender-deep grid place-items-center text-[var(--t-bad)] shadow-sm" title="Remove image">
                    ...
                    {editing ? (
                      ...
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        <span className="text-[11px] font-semibold px-2 py-0.5 rounded-full tabular-nums"
                          style={t.products === 0 ? { background: "var(--s-accent)", color: "var(--t-accent)" } : { background: theme.bg, color: theme.c }}
                          title={`${t.products} product${t.products === 1 ? "" : "s"} carry this tag`}>
                          {t.products}
                        </span>
                        {!t.isActive && <span className="shrink-0 text-[var(--t-warn)]" title="Hidden from the storefront"><Icon name="eye" size={12} /></span>}
                      </div>
                    )}
                  </div>
                  ...
                  {!editing && (
                    <div className="flex items-center gap-1 shrink-0">
                      ...
                      <div className="flex items-center gap-1 opacity-0 group-hover/tag:opacity-100 focus-within:opacity-100 transition-opacity">
                        ...
                        <button onClick={() => onRemoveTag(t)} className="w-[28px] h-[28px] rounded-[8px] grid place-items-center text-[var(--t-bad)] hover:bg-[var(--s-bad)]" title="Delete"><Icon name="trash" size={14} /></button>
                      </div>
      ...
      {/* storefront preview */}
      <div className="border-t border-lavender-deep px-5 py-4" style={{ background: "linear-gradient(180deg,var(--a-solid),var(--a-solid))" }}>
