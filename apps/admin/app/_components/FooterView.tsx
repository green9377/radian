          ...
          <div className="flex-1 min-w-0">
            ...
            {loading ? <p className="text-[13px] text-body-soft">Loading…</p> : (
              <>
                      ...
                      {shownGroups.map((g, gi) => (
                        <div key={g.id} className="border border-lavender-deep rounded-[16px] bg-white p-4">
                          <div className="flex items-center gap-2 mb-3">
                            ...
                            <button onClick={() => removeGroup(g.id)} className="text-[12.5px] text-body-soft hover:text-[#e1837a]">Remove column</button>
                          </div>
                            ...
                            {g.links.map((l, li) => (
                              <div key={l.id} className="grid gap-2 items-center" style={{ gridTemplateColumns: "1fr 1fr auto" }}>
                                ...
                                <div className="flex items-center gap-1">
                                  ...
                                  <button onClick={() => removeLink(l.id, g.id)} className="text-body-soft hover:text-[#e1837a] p-1"><Icon name="trash" size={14} /></button>
                                </div>
                              </div>
                            ))}
                          </div>
                      ...
                      {socials.map((so, i) => (
                        <div key={so.id} className="border border-lavender-deep rounded-[12px] bg-white px-3 py-2.5 grid gap-2 items-center" style={{ gridTemplateColumns: "150px 160px 1fr auto" }}>
                          ...
                          <div className="flex items-center gap-1">
                            ...
                            <button onClick={() => removeSocial(so.id)} className="text-body-soft hover:text-[#e1837a] p-1"><Icon name="trash" size={14} /></button>
                          </div>
                        </div>
                      ))}
                    </div>
                ...
                {sec === "badges" && (
                  <>
                    <p className="text-[12px] text-[#ecc779] bg-[#3b3117] border border-[#534528] rounded-[10px] px-3.5 py-2.5 mb-3">
                      ...
                      {badges.map((b, i) => (
                        <div key={b.id} className="border border-lavender-deep rounded-[12px] bg-white p-2.5 w-[176px]">
                          ...
                          <div className="flex items-center justify-between mt-1.5">
                            ...
                            {b.imageUrl && <button onClick={() => patchBadge(b.id, { imageUrl: null })} className="text-[11.5px] text-body-soft hover:text-[#e1837a]">clear</button>}
                            <button onClick={() => removeBadge(b.id)} className="text-body-soft hover:text-[#e1837a]"><Icon name="trash" size={13} /></button>
                          </div>
                        </div>
                      ))}
                    </div>
