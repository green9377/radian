...
const LOOKS: Record<string, { deep: string; tint: string; icon: string }> = {
  CASH:  { deep: "var(--t-ok)", tint: "var(--t-ok)", icon: "cash" },
  BKASH: { deep: "var(--t-orchid)", tint: "var(--t-orchid)", icon: "phone" },
  NAGAD: { deep: "var(--t-warn)", tint: "var(--t-warn)", icon: "phone" },
  CARD:  { deep: "var(--t-info)", tint: "var(--t-info)", icon: "register" },
  BANK:  { deep: "var(--t-info)", tint: "var(--t-info)", icon: "warehouse" },
  OTHER: { deep: "var(--t-bad)", tint: "var(--t-bad)", icon: "wallet" },
};
      ...
      <div className="md:columns-2 gap-5">
              ...
              <div className="flex items-center justify-between gap-3 px-4 py-3.5"
                ...
                <button type="button" disabled={busy === r.id} onClick={() => toggleMethod(r)}
                  ...
                  style={{
                    ...
                    background: r.isActive ? look.deep : "var(--s-accent)",
                    ...
                    <span className="flex items-center gap-0.5 shrink-0">
                      ...
                      <button type="button" disabled={busy === a.id} onClick={() => toggleAccount(a)}
                        ...
                        style={a.isActive
                          ...
                          : { background: "var(--s-accent)", color: "var(--t-soft)" }}>
                        {a.isActive ? "ON" : "OFF"}
                      </button>
                    </span>
                  </div>
        ...
        {site.length > 0 && (
          <div className="break-inside-avoid mb-5 rounded-[16px] overflow-hidden bg-white shadow-soft border border-lavender-deep">
            <div className="flex items-center gap-3 px-4 py-3.5" style={{ background: "var(--s-accent)" }}>
              <span className="w-[40px] h-[40px] rounded-[12px] grid place-items-center text-white shrink-0" style={{ background: "var(--s-accent)" }}>
                <Icon name="store" size={19} />
              </span>
              <span>
                <span className="block text-[16.5px] font-semibold leading-[1.2]" style={{ color: "var(--t-accent)" }}>The website&apos;s own</span>
...
function AccountDialog({ state, onClose, onSaved }: {
  ...
  return (
    <Modal
      ...
      {err && <div className="mb-3 text-[12.5px] text-[var(--t-bad)] bg-[var(--s-bad)] border border-[var(--l-bad)] rounded-[10px] px-3 py-2">{err}</div>}
        ...
        <div className="mt-4 pt-3 border-t border-lavender-deep flex items-center justify-between gap-3">
          ...
          <button type="button" disabled={busy} onClick={remove}
            className={"text-[12.5px] font-semibold px-3 py-2 rounded-[9px] shrink-0 " + (armDelete
              ? "text-white"
              : "text-[var(--t-bad)] border border-[var(--l-bad)] bg-white hover:bg-[var(--s-bad)]")}
            style={armDelete ? { background: "var(--s-bad)" } : undefined}>
